...
import numpy as np
import matplotlib.pyplot as plt
u = 0.5 # initial value of synaptic resource
x = 0.5 # initial value of utilization variable
tau_rec = 100 # time constant of recovery
dt = 0.1 # time step
T = 1000 # total simulation time
time_steps = np.arange(0, T, dt)
u_values = np.zeros(time_steps.size)
x_values = np.zeros(time_steps.size)
u_values[0] = u
x_values[0] = x
for i, t in enumerate(time_steps[1:], 1):
    # calculate the change in u and x
    du = (-u + (1 - u) * x) / tau_rec * dt
    dx = (-x / tau_rec + (1 - x) * u) * dt
    
    # update the values of u and x
    u_values[i] = u_values[i-1] + du
    x_values[i] = x_values[i-1] + dx
plt.plot(time_steps, u_values, label='u')
plt.plot(time_steps, x_values, label='x')
plt.xlabel('time')
plt.ylabel('value')
plt.legend()
plt.show()
...
tau_r = 0.002  # Recovery time constant
tau_d = 0.8  # Depression time constant

def conductance(tau_r, tau_d, num_spikes):
    # Initialize conductance and recovery rate
    g = 0
    r = 1
    
    # Calculate conductance for each spike
...
interact(conductance, tau_r=(0.001, 0.005, 0.001), tau_d=(0.5, 1, 0.1), num_spikes=(1, 10, 1))
